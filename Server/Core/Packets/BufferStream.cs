...
namespace Game_Maker_Studio_2_Auth_Server.Core.Packets
{
    /// <summary>
    ...
    public class BufferStream
    {
        ...
        /// <summary>
        /// Instantiates an instance of the BufferStream class with an existing byte array and alignment.
        /// </summary>
        /// <param name="data">Byte array to initialize the BufferStream with.</param>
        /// <param name="alignment">Alignment of the BufferStream in bytes.</param>
        public BufferStream(byte[] data, int alignment)
        {
            fastAlign = alignment - 1;
            fastAlignNot = ~fastAlign;
            this.length = data.Length;
            this.alignment = alignment;
            memory = new byte[AlignedIterator(data.Length, alignment)];
        ...
        /// <summary>
        /// Reads a UInt16 from the current position and advances the iterator.
        /// </summary>
        public ushort ReadUInt16()
        {
            if (iterator + 2 > memory.Length)
            {
                throw new InvalidOperationException("Attempt to read beyond buffer length.");
            }

            ushort value = BitConverter.ToUInt16(memory, iterator);
            iterator += 2;
            return value;
        }
        ...
        public byte[] ToArray()
        {
            ...
            Array.Copy(memory, result, Length);
            return result;
        }
